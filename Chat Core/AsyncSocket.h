...
/* Once one of these methods is called, the AsyncSocket instance is locked in, and the rest can't be called without disconnecting the socket first. If the attempt times out or fails, these methods either return NO or call "socket:willDisconnectWithError:" and "onSockedDidDisconnect:". */
...
/* The following methods won't block. To not time out, use a negative time interval. If they time out, "socket:disconnectWithError:" is called. The tag is for your convenience. You can use it as an array index, step number, state id, pointer, etc., just like the socket's user data. */
