...
- (void) addMessageToDisplay:(NSData *) message fromUser:(MVChatUser *) user withAttributes:(NSDictionary *) msgAttributes withIdentifier:(NSString *) identifier andType:(JVChatMessageType) type {
	...
	if( ! [user isLocalUser] ) {
		...
		for( __strong NSString *name in names ) {
			...
			NSArray *matches = [regex matchesInString:[messageString string] options:NSMatchingReportCompletion range:NSMakeRange( 0, [messageString string].length ) ];
...
- (void) _hyperlinkRoomNames:(NSMutableAttributedString *) message {
	...
	NSArray *matches = [regex matchesInString:[message string] options:NSMatchingReportCompletion range:NSMakeRange( 0, [message string].length )];
