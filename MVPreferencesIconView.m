...
const unsigned int bottomBorder = 13;

#pragma mark -

@implementation MVPreferencesIconView
...
- (void) setSelectedPane:(NSBundle *) newSelectedPane {
	[selectedPane autorelease];
	selectedPane = [newSelectedPane retain];
	[self setNeedsDisplay:YES];
}
	...
	if( [chars length] && [chars characterAtIndex:0] == NSTabCharacter ) {
		...
		if( focusedIndex >= [self _numberOfIcons] ) {
			if( ( nextView = [self nextValidKeyView] ) ) {
				...
				focusedIndex = NSNotFound;
				if( [nextView isKindOfClass:[MVPreferencesIconView class]] )
					[(MVPreferencesIconView *)nextView _focusFirst];
			} else focusedIndex = 0;
		}
		...
		if( (signed) focusedIndex < 0 && [self _numberOfIcons] ) {
			if( ( nextView = [self previousValidKeyView] ) ) {
				...
				focusedIndex = NSNotFound;
				if( [nextView isKindOfClass:[MVPreferencesIconView class]] )
					[(MVPreferencesIconView *)nextView _focusLast];
...
- (void) drawRect:(NSRect) rect {
	unsigned int paneIndex = 0, paneCount = 0;
...
@implementation MVPreferencesIconView (MVPreferencesMultipleIconViewPrivate)
- (void) _focusFirst {
	focusedIndex = 0;
	[self setNeedsDisplayInRect:[self _boundsForIndex:focusedIndex]];
}
...
- (void) _drawIconAtIndex:(unsigned int) index drawRect:(NSRect) drawRect {
	...
	float nameHeight = 0.;
	NSRect buttonRect = NSZeroRect, destinationRect = NSZeroRect;
	...
	if( index != pressedIconIndex && image ) [image drawFlippedInRect:destinationRect operation:NSCompositeSourceOver fraction:1.];
		...
		NSImage *darkImage = nil;
		NSSize darkImageSize = NSZeroSize;
