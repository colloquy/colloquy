...
static void // mutex argument added by T. Hatcher to optionally lock around callbacks (thread safety)
g_main_dispatch (GTimeVal *dispatch_time, pthread_mutex_t *lock)
{
  while (pending_dispatches != NULL)
      ...
      if (G_HOOK_IS_VALID (source))
	{
	  ...
	  G_UNLOCK (main_loop);
	  if( lock ) pthread_mutex_lock( lock ); // added by T. Hatcher to optionally lock around callbacks (thread safety)
	  need_destroy = ! dispatch (source_data,
				     dispatch_time,
				     hook_data);
	  if( lock ) pthread_mutex_unlock( lock ); // added by T. Hatcher to optionally lock around callbacks (thread safety)
	  G_LOCK (main_loop);
 ...
 */
static gboolean
g_main_iterate (gboolean block, // mutex argument added by T. Hatcher to optionally lock around callbacks (thread safety)
		gboolean dispatch, pthread_mutex_t *lock)
{
  GHook *hook;
  ...
  if (pending_dispatches)
    {
      if (dispatch)
	g_main_dispatch (&current_time, lock);
...
g_main_pending (void)
{
  return in_check_or_prepare ? FALSE : g_main_iterate (FALSE, FALSE, NULL);
}

/* Run a single iteration of the mainloop. If block is FALSE,
 * will never block
 */
gboolean // mutex argument added by T. Hatcher to optionally lock around callbacks (thread safety)
g_main_iteration (gboolean block, pthread_mutex_t *lock)
{
  if (in_check_or_prepare)
    ...
    return g_main_iterate (block, TRUE, lock);
}
...
void // mutex argument added by T. Hatcher to optionally lock around callbacks (thread safety)
g_main_run (GMainLoop *loop, pthread_mutex_t *lock)
{
