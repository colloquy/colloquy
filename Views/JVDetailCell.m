...
- (void) drawWithFrame:(NSRect) cellFrame inView:(NSView *) controlView {
	...
	if( ! [self isEnabled] && [self image] ) {
		...
		[[self image] drawAtPoint:NSMakePoint( 0., 0. ) fromRect:NSZeroRect operation:NSCompositeCopy fraction:.5];
...
#define JVDetailCellStatusImageRightPadding JVDetailCellStatusImageLeftPadding
	...
	if( _statusImage && NSHeight( cellFrame ) >= [_statusImage size].height ) {
		[_statusImage drawAtPoint:NSMakePoint( NSMaxX( cellFrame ) - statusWidth, NSMaxY( cellFrame ) - ( ( NSHeight( cellFrame ) / 2 ) - ( [_statusImage size].height / 2 ) ) ) fromRect:NSZeroRect operation:NSCompositeSourceAtop fraction:( [self isEnabled] ? 1. : 0.5)];
	}
}
