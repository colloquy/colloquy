...
- (void) parseStyleOptions {
	...
	for( NSMutableDictionary *info in _styleOptions ) {
		...
		for( NSString *style in sarray ) {
			...
			for( NSTextCheckingResult *selector in [regex matchesInString:style options:NSMatchingReportCompletion range:NSMakeRange( 0, style.length )] ) {
				...
				for( NSTextCheckingResult *property in [regex matchesInString:matchedText options:NSMatchingReportCompletion range:NSMakeRange( 0, matchedText.length )] ) {
					...
					if( [info[@"type"] isEqualToString:@"list"] ) {
						...
						if( value && [v rangeOfString:@"%@"].location != NSNotFound ) {
							...
							NSTextCheckingResult *vmatch = [regex firstMatchInString:value options:NSMatchingReportCompletion range:NSMakeRange( 0, value.length )];
...
- (NSString *) valueOfProperty:(NSString *) property forSelector:(NSString *) selector inStyle:(NSString *) style {
	...
	NSTextCheckingResult *match = [regex firstMatchInString:style options:NSMatchingReportCompletion range:NSMakeRange( 0, style.length ) ];
